...
import com.parkinfo.service.archiveInfo.impl.DeclarePaperService;
import com.parkinfo.service.archiveInfo.impl.SpotCheckService;
...
@RestController
@RequestMapping("/archiveInfo/spotCheck")
@Api(value = "/archiveInfo/spotCheck", tags = {"存档资料-点检记录"})
public class SpotCheckController {

    @Autowired
    private SpotCheckService archiveInfoService;

    @PostMapping("/search")
    @RequiresPermissions("archiveInfo:spotCheck:search")
    @ApiOperation(value = "根据查询条件分页查询文件")
    public Result<Page<ArchiveInfoResponse>> search(@RequestBody QueryArchiveInfoRequest request){
        request.setGeneral("点检记录");
    ...
    @RequiresPermissions(value = "archiveInfo:spotCheck:delete")
    ...
    @PostMapping("/add")
    @RequiresPermissions(value = "archiveInfo:spotCheck:add")
    ...
    @PostMapping("/edit/{id}")
    @RequiresPermissions(value = "archiveInfo:spotCheck:edit")
    ...
    @RequiresPermissions(value = "archiveInfo:spotCheck:download")
    ...
    public Result<List<ArchiveInfoTypeResponse>> findAllType(){
        return archiveInfoService.findAllType("点检记录");
    }
