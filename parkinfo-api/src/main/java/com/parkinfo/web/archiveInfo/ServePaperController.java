...
import com.parkinfo.service.archiveInfo.impl.ServePaperService;
...
@RestController
@RequestMapping("/archiveInfo/serve")
@Api(value = "/archiveInfo/serve", tags = {"存档资料-服务体系"})
public class ServePaperController {

    @Autowired
    private ServePaperService archiveInfoService;

    @PostMapping("/search")
    @RequiresPermissions("archiveInfo:serve:search")
    @ApiOperation(value = "根据查询条件分页查询文件")
    public Result<Page<ArchiveInfoResponse>> search(@RequestBody QueryArchiveInfoRequest request){
        request.setGeneral("服务体系");
    ...
    @RequiresPermissions(value = "archiveInfo:serve:delete")
    ...
    @PostMapping("/edit/{id}")
    @RequiresPermissions(value = "archiveInfo:serve:edit")
    ...
    @RequiresPermissions(value = "archiveInfo:serve:download")
    ...
    public Result<List<ArchiveInfoTypeResponse>> findAllType(){
        return archiveInfoService.findAllType("服务体系");
    }
